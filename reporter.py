...
METODOS = {
    ...
    "4":  ("Abuso Infantil",     types.InputReportReasonChildAbuse,       "Este canal tiene contenido de abuso infantil"),
    "5":  ("Otro",               types.InputReportReasonOther,            None),
    "6":  ("Derechos de Autor",  types.InputReportReasonCopyright,        "Este canal viola derechos de autor"),
    "7":  ("Falso/Suplantación", types.InputReportReasonFake,             "Este canal es falso o suplanta identidad"),
    "8":  ("Geo Irrelevante",    types.InputReportReasonGeoIrrelevant,    "Contenido geográficamente irrelevante"),
    "9":  ("Drogas Ilegales",    types.InputReportReasonIllegalDrugs,     "Este canal promueve drogas ilegales"),
    "10": ("Datos Personales",   types.InputReportReasonPersonalDetails,  "Este canal filtra datos personales"),
}
...
def main():
    ...
    objetivo = input(f"{lrd}[{lgn}+{lrd}] {gn}@canal o link : {k}").strip()
    try:
    ...
    try:
        with TelegramClient('session_reporter', api_id, api_hash) as client:
